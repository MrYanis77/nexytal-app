import formationsIndex from './json/formations-index.json';

const SOURCE_LOADERS = {
  formation: () => import('./json/formation.json'),
  'formation-courtes': () => import('./json/formation-courtes.json'),
  'formations-certifiantes': () => import('./json/formations-certifiantes.json'),
};
...
function formatFormationEntry(id, data, sourceKey) {
  ...
  const type =
    sourceKey === 'formation-courtes'
      ? data.type || 'courte'
      : sourceKey === 'formations-certifiantes'
...
/**
 * Charge une fiche formation par id (import dynamique d'un seul fichier JSON source).
 */
export async function getFormationById(id) {
  const sourceKey = formationsIndex[id];
  if (!sourceKey || !SOURCE_LOADERS[sourceKey]) return null;

  const mod = await SOURCE_LOADERS[sourceKey]();
  const raw = mod.default[id];
  if (!raw) return null;

  return formatFormationEntry(id, raw, sourceKey);
}
